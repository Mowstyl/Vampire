...
public class MLang {
	...
	public static MLang load(VampireRevamp plugin) {
		MLang i = null;
		File dataFolder = plugin.getDataFolder();
		File configFile = new File(dataFolder, "mlang.json");
		...
		try {
			if (!configFile.exists()) {
				...
				try {
					...
					i = gson.fromJson(reader, MLang.class);
					reader.close();
				}
				catch (Exception ex) {
					Bukkit.getLogger().log(Level.SEVERE, "Couldn't parse mlang.json. Resetting to defaults!");
					i = new MLang();
					Writer writer = new FileWriter(configFile);
					gson.toJson(i, writer);
					writer.flush();
	...
	public String boolIsY(String boolName, boolean val) { return TextUtil.parse(xIsY, boolName, val ? on : off); }
	public String quotaIsPercent(String quotaName, double val) { return TextUtil.parse(xIsY, quotaName, percent(val)); }
	public String percent(double quota) { return TextUtil.parse("%.1f%%", quota*100); }
	...
	public List<String> infectionFeeling = ImmutableList.of(
	...
	public List<String> infectionHint = ImmutableList.of(
